...
export default function Voting({ hotspot }) {
  ...
  const handleUpvote = () => {
    ...
    // if user vote is already upvote, undo it
    if (userVote === 'upvote') {
      setUserVote('null');
      if (user) {
        console.log('undoing upvote')
        downvoteHotspot(hotspot.hotSpotID, user.userID, true)
      }
      return;
    }

    // if user vote was a downvote, undo it and upvote
    if (userVote === 'downvote') {
      setUserVote('upvote');
      if (user) {
        console.log('undoing downvote and upvoting');
        upvoteHotspot(hotspot.hotSpotID, user.userID, true);
        upvoteHotspot(hotspot.hotSpotID, user.userID, false);
      }
  ...
  const handleDownVote = () => {
    ...
    // if user vote is already downvote, undo it
    if (userVote === 'downvote') {
      setUserVote('null');
      if (user) {
    ...
    // if user vote was a upvote, undo it and upvote
    if (userVote === 'upvote') {
      setUserVote('downvote');
      if (user) {
        console.log('undoing upvote and downvoting');
        downvoteHotspot(hotspot.hotSpotID, user.userID, true);
        downvoteHotspot(hotspot.hotSpotID, user.userID, false);
      }
